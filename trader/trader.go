...
// Switch ...
func Switch(user model.User,id int64) (err error) {
    if GetTraderStatus(id) > 0 {
      fmt.Println("Stop a Trader")
        return stop(user, id)
    }
    fmt.Println("Start a Trader")
    return run(user, id)
}
...
// run ...
func run(user model.User, id int64) (err error) {
    ...
    go func() {
        defer func() {
            ...
            fmt.Println("Set Status as 0")
            trader.Status = 0
        }()
        trader.LastRunAt = time.Now()
        fmt.Println("Set Status as 1")
        trader.Status = 1
  ...
  // user.UpdateTrader(trader.Trader)
    return
}
...
// stop ...
func stop(user model.User,id int64) (err error) {
    if t, ok := Executor[id]; !ok || t == nil {
